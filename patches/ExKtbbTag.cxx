...
bool ExKtbbTag::CalculateInputVariables(xAOD::Jet& jetToTag, xAOD::BTagging* BTag){
  ...
  auto v_el = jetToTag.auxdata<std::vector<ElementLink<xAOD::IParticleContainer> > >(m_SubJetLabel);
  for(auto el : v_el){
    if(!el.toTransient()){
      ATH_MSG_WARNING("Unable to reset element link in " << m_SubJetLabel << ". You would crash soon ...");
    }

    if(!el.isValid()){
      ATH_MSG_ERROR("Invalid link to subjet through link " << m_SubJetLabel << " !");
      return false;
    }
    else{
